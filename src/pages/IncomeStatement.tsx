...
export function IncomeStatement() {
  ...
  return (
    <div className="max-w-[720px] print:max-w-none">
      ...
      <div className="mb-3 text-[12px] font-normal text-muted-60 print:hidden">Derived live from recorded sales margin and journal postings, for the period you select.</div>
      ...
      <div className="mb-3 text-[11.5px] font-normal text-muted-60 print:hidden">
      ...
      <Card className="overflow-hidden print:border-0 print:shadow-none">
        ...
        <div className="flex items-center gap-2.5 border-b border-divider bg-surface-sunken px-[13px] py-2.5">
          <div className="flex-1">
            ...
            <div className="text-[11px] font-normal text-muted-60">Margin on currency sales plus journal postings to Income</div>
          </div>
        ...
        <div className="flex items-center gap-2.5 border-b border-divider py-2 pl-5 pr-[13px]">
          <div className="flex-1">
            ...
            <div className="text-[11px] font-normal text-muted-60">{result.currencyRows.length} desk{result.currencyRows.length === 1 ? '' : 's'} with activity in this period</div>
          </div>
        ...
        {result.currencyRows.map((cur) => (
          <div key={cur.code} className="flex items-center gap-2.5 border-b border-divider bg-surface-sunken py-2 pl-[26px] pr-[13px]">
            <div className="flex-1">
              <div className="text-[12px] font-medium">{cur.code} desk</div>
              <div className="text-[11px] font-normal text-muted-60">{cur.sub}</div>
            </div>
            <div className={`tabular min-w-[150px] text-right text-[12.5px] font-medium ${cur.margin >= 0 ? '' : 'text-negative'}`}>{fmt(cur.margin)}</div>
          </div>
        ))}
          ...
          <>
            <div className="flex items-center gap-2.5 border-b border-divider py-2 pl-5 pr-[13px]">
              <div className="flex-1">
                ...
                <div className="text-[11px] font-normal text-muted-60">{result.adjRows.length} entr{result.adjRows.length === 1 ? 'y' : 'ies'}</div>
              </div>
            ...
            {result.adjRows.map((row, i) => (
              <div key={i} className="flex items-center gap-2.5 border-b border-divider py-1.5 pl-[26px] pr-[13px]">
                <div className="flex-1">
                  <div className="text-[12px] font-normal">{row.label}</div>
                  <div className="text-[11px] font-normal text-muted-60">{row.sub}</div>
                </div>
                <div className="tabular min-w-[150px] text-right text-[12px] font-normal text-muted-70">{fmt(row.amount)}</div>
              </div>
            ))}
          </>
        )}
        ...
        <div className="flex items-center gap-2.5 border-b border-divider bg-surface-sunken px-[13px] py-2.5">
          <div className="flex-1">
            ...
            <div className="text-[11px] font-normal text-muted-60">{result.expenseRows.length} account{result.expenseRows.length === 1 ? '' : 's'} with postings in this period</div>
          </div>
        ...
        <div className="flex items-center gap-2.5 border-b border-divider bg-surface-sunken px-[13px] py-2.5">
          <div className="flex-1">
            ...
            <div className="text-[11px] font-normal text-muted-60">Accrued salary expense inside this period (included in Expenses above)</div>
          </div>
        ...
        <div className="flex items-center gap-2.5 border-t-2 border-border-input bg-surface-hover px-[13px] py-2.5">
          <div className="flex-1">
            ...
            <div className="text-[11px] font-normal text-muted-60">Gross profit less expenses (the salary accrual is inside Expenses)</div>
          </div>
      ...
      {failed.length > 0 ? (
        <div className="mt-3 rounded-[6px] border border-negative-border border-l-[3px] border-l-negative bg-negative-bg px-3 py-2.5">
          <div className="mb-1 flex items-center gap-1.5 text-[12px] font-bold text-negative-deep">
          ...
          {failed.map((c, i) => (
            <div key={i} className="mt-1.5">
              ...
              <div className="text-[11px] font-normal leading-relaxed text-negative-deep">{c.detail}</div>
            </div>
          ))}
        </div>
      ) : (
        <div className="mt-3 flex items-start gap-1.5 text-[11.5px] font-normal leading-relaxed text-muted-60">
