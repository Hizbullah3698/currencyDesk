...
export type AppState = PersistShape
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  // cleared once, here, for the whole store. Identity (who's signed in, what role they have)
  // lives in AuthProvider/useAuth, not here, so it never touches `pristine` at all.
  const mutate = (fn: (s: PersistShape) => PersistShape) => setPersisted((s) => ({ ...fn(s), pristine: false }))

  const state: AppState = persisted
  // Fails open toward admin while `user` is null (during the initial session check, or once
  // signed out) — mirrors this app's pre-existing loose `role !== 'user'` pattern. Harmless
  // only because Gate() never renders any protected route while `user` is null; that's
  // incidental, not a deliberate second layer of defense.
