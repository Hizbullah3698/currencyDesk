    ...
    <div>
      ...
      <Card variant="flat" className="mb-4 grid grid-cols-[1fr_1fr_1fr_1.4fr] items-end gap-5 p-4 print:shadow-none">
        ...
        <div>
          ...
          <div className="tabular text-body font-medium">{fmtRate(aed.avgCost)}</div>
        </div>
        <div>
        ...
        <Card className="overflow-hidden">
          ...
          {purchases.map((r) => (
            <div key={r.id} onClick={() => r.customerId && navigate(`/customers/${r.customerId}`)} className="flex cursor-pointer items-center gap-2 border-b border-divider px-[13px] py-2 transition-colors duration-150 hover:bg-surface-hover">
              <div className="min-w-0 flex-1 truncate text-body font-semibold">{r.customerName}</div>
          ...
          {purchases.length === 0 && (
            <div className="flex flex-col items-center gap-1.5 py-5 text-center">
              ...
              <div className="text-body font-normal text-muted-60">No purchases yet.</div>
            </div>
          )}
        </Card>
        <Card className="overflow-hidden">
          <div className="border-b border-border px-[13px] py-2.5 text-body font-semibold">Recent sales</div>
          {sales.map((r) => (
            <div key={r.id} onClick={() => r.customerId && navigate(`/customers/${r.customerId}`)} className="flex cursor-pointer items-center gap-2 border-b border-divider px-[13px] py-2 transition-colors duration-150 hover:bg-surface-hover">
              <div className="min-w-0 flex-1 truncate text-body font-semibold">{r.customerName}</div>
              <div className="tabular text-meta font-normal text-muted-60">
          ...
          {sales.length === 0 && (
            <div className="flex flex-col items-center gap-1.5 py-5 text-center">
              <Inbox size={16} strokeWidth={1.8} className="text-muted-42" aria-hidden="true" />
              <div className="text-body font-normal text-muted-60">No sales yet.</div>
            </div>
          )}
        </Card>
      </div>
      ...
      <Card className="mb-4 overflow-hidden print:shadow-none">
        ...
        {codes.map((code) => {
          ...
          return (
            <div key={code} className="flex items-center gap-2.5 border-b border-divider px-[13px] py-2.5 transition-colors duration-150 hover:bg-surface-hover">
              <div className="min-w-[70px] text-body font-semibold">{code}</div>
              <div className="tabular flex-1 text-right text-body font-normal">{fmtNum(s.available)}</div>
              <div className="tabular min-w-[130px] text-right text-body font-normal">{fmtRate(s.avgCost)}</div>
        ...
        <div className="px-[13px] py-2 text-meta font-normal text-muted-60">Each row is carried separately on the Balance Sheet as its own Currency stock account, at its own weighted-average cost.</div>
      </Card>
      ...
      {!isAdmin ? (
        ...
        <Card className="overflow-hidden print:shadow-none">
          <div className="border-b border-border px-[13px] py-2.5">
            <div className="text-body font-semibold">Currency ledger</div>
            <div className="mt-0.5 text-meta font-normal text-muted-60">Every movement in and out of stock, oldest first — purchases re-weight the average cost, sales draw down at it.</div>
          </div>
          <div className="flex items-center gap-2.5 border-b border-border bg-surface-sunken px-[13px] py-[7px] text-meta font-semibold uppercase tracking-wide text-muted-60">
          ...
          {ledgerRows.map((row, i) => (
            <div key={i} className="flex items-center gap-2.5 border-b border-divider px-[13px] py-2 transition-colors duration-150 hover:bg-surface-hover">
              ...
              <div className={`tabular min-w-[110px] text-right text-body font-normal ${row.type === 'purchase' ? 'text-positive' : 'text-negative'}`}>
                {row.type === 'purchase' ? '+' : '−'}
                {fmtNum(row.amount)}
              </div>
              <div className="tabular min-w-[80px] text-right text-body font-normal text-muted-70">{fmtRate(row.rate)}</div>
              <div className="tabular flex-1 text-right text-body font-normal">{fmtNum(row.runQty)}</div>
              <div className="tabular min-w-[130px] text-right text-body font-semibold">{fmtRate(row.runAvg)}</div>
            </div>
          ))}
