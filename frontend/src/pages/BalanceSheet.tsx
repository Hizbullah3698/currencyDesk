...
export function BalanceSheet() {
  ...
  return (
    <div>
      ...
      <div className="mb-3 text-body font-normal text-muted-60 print:hidden">
      ...
      <div className="mb-3 text-meta font-normal text-muted-60 print:hidden">As of {bounds.label}.</div>
      ...
      <Card className="overflow-hidden print:border-0 print:shadow-none">
        ...
        {result.groups.map((group) => {
          ...
          <div key={group.title}>
            ...
            {group.rows.map((row) => (
              <div key={row.id} className="flex items-center gap-2.5 border-b border-divider px-[13px] py-2 transition-colors duration-150 hover:bg-surface-hover">
                <div className="flex-1">
                  <div className="text-body font-semibold">{row.label}</div>
                  <div className="text-meta font-normal text-muted-60">{row.sub}</div>
                </div>
                <div className="tabular min-w-[130px] text-right text-body font-medium text-ink">{row.dr ? fmt(row.dr) : ''}</div>
                <div className="tabular min-w-[130px] text-right text-body font-medium text-muted-70">{row.cr ? fmt(row.cr) : ''}</div>
              </div>
            ))}
            <div className="flex items-center gap-2.5 border-b border-border bg-surface-sunken px-[13px] py-1.5">
              <div className="flex-1 text-meta font-normal text-muted-60">Subtotal</div>
              <div className="tabular min-w-[130px] text-right text-body font-semibold text-ink">{fmt(group.subDr)}</div>
              <div className="tabular min-w-[130px] text-right text-body font-semibold text-muted-70">{fmt(group.subCr)}</div>
            </div>
          </div>
          )
        })}
        <div className="flex items-center gap-2.5 border-t-2 border-border-input bg-surface-hover px-[13px] py-2.5">
          <div className="flex-1 text-body font-bold">Total</div>
          <div className="tabular min-w-[130px] text-right text-body font-bold text-ink">{fmt(result.totalDr)}</div>
          <div className="tabular min-w-[130px] text-right text-body font-bold text-muted-70">{fmt(result.totalCr)}</div>
        </div>
      </Card>
          ...
          {result.diags.map((d, i) => (
            <div key={i} className="flex items-start gap-3 border-b border-divider px-[13px] py-2.5 transition-colors duration-150 hover:bg-surface-hover">
              <div className="flex-1">
                ...
                <div className="text-meta font-normal leading-relaxed text-muted-70">{d.detail}</div>
              </div>
              <div className="tabular min-w-[120px] text-right text-body font-semibold">{fmt(d.amount)}</div>
            </div>
          ))}
        </Card>
      )}
